 ...
 * @author  jpoon
 */
public class AuthStorageCookie implements AuthStorage {

    /** Creates a new instance of AuthStorageCookie */
    public AuthStorageCookie() {
    }

    public AuthStorageCookie(PageContext pageContext) {
    ...
    public void store() {
       ...
       //remoteUser
       Cookie cookie=new Cookie("remoteUser", getRemoteUser());
       ...
       //authCode
       cookie=new Cookie("authCode", getAuthCode());
       cookie.setPath("/");
       if(timeToLiveSeconds>=0)
           cookie.setMaxAge(timeToLiveSeconds);
       response.addCookie(cookie);
    ...
    public void load() {  
       ...
       if(cookies!=null) {
           for(int i=0; i<cookies.length; i++) {
               ...
               if(cookieName.equalsIgnoreCase("remoteUser")) 
                   setRemoteUser(cookies[i].getValue());
               if(cookieName.equalsIgnoreCase("authCode")) 
                   setAuthCode(cookies[i].getValue());               
           }
       }
    }
     ...
     */
    public void remove() {
        String time=getTimeToLiveSeconds();
        store("", AuthSource.DENIED, "0");    
        setTimeToLiveSeconds(time);
    }
    ...
    public void setTimeToLiveSeconds(String timeToLiveSeconds) {
        try {
            ...
            System.out.println("AuthStorageCookie.setTimeToLiveSeconds: "+exception);
   ...
   //***************************************************************************  
   private final static String  AUTH_NAME="cookie";
   private final static int     DEFAULT_TTL=-1;
